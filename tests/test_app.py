...
class TestQuestionEndpoints:
    def test_add_text_question(self, client):
        ...
        response = client.post(
            ...
            data=json.dumps({"type": "text", "text": "What is your name?"}),
            content_type="application/json",
        )
        assert response.status_code == 201
        data = json.loads(response.data)
...
class TestSurveyWorkflow:
    def test_complete_survey_workflow(self, client):
        survey_response = client.post(
            "/surveys",
            data=json.dumps(
                {"title": "Customer Satisfaction", "description": "Annual survey"}
            ),
            content_type="application/json",
        )
        ...
        publish_response = client.post(f"/surveys/{survey_id}/publish")
